...
def generate_unique_short_url_and_update_db(request, long_url: str, is_permanent: bool = True) -> ShortUrl:
    """
    create short url for original url
    adds long_url to db if it does not exist

    adds short link to db
    returns ShortUrl if action is successful
    """
    ...
    for i in range(MAX_NUMBER_OF_ATTEMPTS):
        ...
        try:
            ...
            # add short url to db
            return store_short_url(short_url, original_url, is_permanent)
